...
describe("validateCreateTodoInput", () => {
  it("trims valid titles and parses due dates to end-of-day", () => {
    ...
    expect(result.value.dueDate?.getUTCHours()).toBe(23);
    expect(result.value.dueDate?.getUTCMinutes()).toBe(59);
    expect(result.value.dueDate?.getUTCSeconds()).toBe(59);
  });
